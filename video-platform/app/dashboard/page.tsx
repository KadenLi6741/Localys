...
function DashboardContent() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-24">
      {/* Header */}
      <div className="bg-white dark:bg-card border-b border-gray-200 dark:border-border px-4 lg:px-10 pt-3 pb-0">
        <div className="max-w-screen-2xl mx-auto">
          <div className="flex items-end justify-between mb-2">
            ...
            <button onClick={() => { setScanResult(null); setShowScanner(true); }} className="flex items-center gap-1.5 bg-[#f97316] hover:opacity-90 text-white text-sm font-semibold px-3 py-1.5 rounded-lg transition-opacity mb-1">
      ...
      <div className="max-w-screen-2xl mx-auto px-4 lg:px-10 pt-2 pb-4">
        {scanResult && (
          <div className={`mb-2 p-3 rounded-lg border flex items-start justify-between ${scanResult.success ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
        ...
        {/* OVERVIEW */}
        {activeTab === 'overview' && (
          // Calm, gapped layout: soft cards separated by a consistent 12px
          // rhythm so each section reads as its own group (real hierarchy).
          <div className="space-y-3">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-3">
                <div className="flex-1 bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2"><div className="w-6 h-6 rounded-lg bg-[#f97316]/10 flex items-center justify-center"><TrendingUp className="h-3.5 w-3.5 text-[#f97316]" /></div><p className="text-xs font-medium text-foreground">Income</p></div>
                ...
                <div className="flex-1 bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2"><div className="w-6 h-6 rounded-lg bg-red-500/10 flex items-center justify-center"><TrendingDown className="h-3.5 w-3.5 text-red-500" /></div><p className="text-xs font-medium text-foreground">Expense</p></div>
            ...
            <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5">
              ...
              <p className="text-[11px] text-muted-foreground mt-2 leading-snug">
                You keep 95% of every sale. Tax (8.25%) is collected from customers and remitted — not part of your earnings.
              </p>
            </div>
              ...
              {mostPopularOrder ? (
                <div className="flex items-center gap-2">
                  {(itemImages[mostPopularOrder.itemId] || itemNameImages[mostPopularOrder.name]) ? (
                    ...
                    <div className="w-11 h-11 rounded-lg bg-[#f97316]/10 flex items-center justify-center shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <img src="/Menu/Jays%20Burger/Classic%20Burger.jpg" alt="Classic Burger" className="w-11 h-11 rounded-lg object-cover shrink-0" onError={(e) => { (e.target as HTMLImageElement).style.display='none'; }} />
                  <div className="leading-tight">
        ...
        {activeTab === 'orders' && (
          <div className="space-y-1.5">
            <button onClick={() => { setScanResult(null); setShowScanner(true); }} className="w-full flex items-center justify-center gap-2 bg-[#f97316] hover:opacity-90 text-white font-semibold py-2.5 rounded-lg transition-opacity">
            ...
            {groupOrders.length > 0 && (
              <Section title="Group Orders" count={groupOrders.length} countColor="gray">
                {groupOrders.map(go => (
                  <div key={go.id} className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-3">
        ...
        {activeTab === 'reviews' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5 text-center">
              ...
              <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5 text-center col-span-2 sm:col-span-1">
            ...
            {reviews.length === 0 ? (
              <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-10 text-center">
              ...
              <div className="space-y-1.5">
                {reviews.map(r => (
                  <div key={r.id} className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5">
...
function StatCard({ label, value, change }: { label: string; value: string; change?: number }) {
  return (
    <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg px-2.5 py-2 text-center">
...
function RevenueChart({ data, total, change }: { data: { label: string; revenue: number }[]; total: number; change: number }) {
  return (
    <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg px-2.5 pt-1.5 pb-2.5">
      <div className="mb-1 flex items-start justify-between">
        ...
        <span className="rounded-lg border border-gray-200 dark:border-border px-2 py-0.5 text-[11px] font-medium text-muted-foreground">30D</span>
      </div>
      <div className="h-44">
...
function OrdersTable({ pending, scheduled, completed, limit }: {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-200 dark:border-border bg-white dark:bg-card">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="border-b border-gray-200 dark:border-border text-[11px] uppercase tracking-wide text-muted-foreground">
            <tr>
              <Th k="id" label="Order #" />
...
function Empty({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-8 text-center">
  ...
  return (
    <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5">
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={handleSave} disabled={saving} className={`flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-lg transition-colors ${saved ? 'bg-[#f97316]/10 text-[#f97316] border border-orange-200' : 'bg-[#f97316] text-white hover:opacity-90'} disabled:opacity-50`}>
          <Save className="h-3.5 w-3.5" />
          {saving ? 'Saving…' : saved ? 'Saved!' : 'Save'}
        </button>
      </div>
      {error && <div className="mb-2 bg-red-50 border border-red-200 text-red-600 rounded-lg p-3 text-sm">{error}</div>}
      <div className="space-y-1.5">
        <div>
...
function LabeledInput({ icon, type, value, onChange, placeholder }: { icon: React.ReactNode; type: string; value: string; onChange: (v: string) => void; placeholder: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="flex-1 bg-muted border border-gray-200 dark:border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder-gray-400 focus:outline-none focus:border-[#f97316]" />
    </div>
  );
}
  ...
  return (
    <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5">
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={save} disabled={saving} className={`flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-lg transition-colors ${saved ? 'bg-[#f97316]/10 text-[#f97316] border border-orange-200' : 'bg-[#f97316] text-white hover:opacity-90'} disabled:opacity-50`}>
          ...
          <div key={day} className="flex items-center gap-2">
            ...
            {!hours[day]?.closed ? (
              <div className="flex items-center gap-2 flex-1">
                <input type="time" value={hours[day]?.open || '09:00'} onChange={e => setHours({ ...hours, [day]: { ...hours[day], open: e.target.value } })} className="flex-1 bg-muted border border-gray-200 dark:border-border rounded-lg px-2 py-1.5 text-sm text-foreground focus:outline-none focus:border-[#f97316]" />
                <span className="text-muted-foreground text-sm">to</span>
                <input type="time" value={hours[day]?.close || '17:00'} onChange={e => setHours({ ...hours, [day]: { ...hours[day], close: e.target.value } })} className="flex-1 bg-muted border border-gray-200 dark:border-border rounded-lg px-2 py-1.5 text-sm text-foreground focus:outline-none focus:border-[#f97316]" />
              </div>
            ) : (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1.5 bg-[#f97316] hover:opacity-90 text-white text-sm font-semibold px-3 py-2 rounded-lg transition-opacity">
          <Plus className="h-4 w-4" /> New Code
        </button>
      </div>

      {showForm && (
        <div className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5 space-y-4">
          <h3 className="text-sm font-semibold text-foreground">Create Promo Code</h3>
          {formError && <div className="bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg p-3">{formError}</div>}
          <div className="grid grid-cols-2 gap-2">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-muted-foreground mb-1">Code</label>
              <input type="text" value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="e.g. SAVE20" maxLength={20} className="w-full bg-muted border border-gray-200 dark:border-border rounded-lg px-3 py-2 text-sm font-mono text-foreground placeholder-gray-400 focus:outline-none focus:border-[#f97316] uppercase" />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Type</label>
              <select value={discountType} onChange={e => setDiscountType(e.target.value as 'percent' | 'fixed')} className="w-full bg-muted border border-gray-200 dark:border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-[#f97316]">
            ...
            <div>
              ...
              <input type="number" value={discountValue} onChange={e => setDiscountValue(e.target.value)} placeholder={discountType === 'percent' ? '20' : '5.00'} min="0" step={discountType === 'percent' ? '1' : '0.01'} className="w-full bg-muted border border-gray-200 dark:border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder-gray-400 focus:outline-none focus:border-[#f97316]" />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Max Uses</label>
              <input type="number" value={maxUses} onChange={e => setMaxUses(e.target.value)} placeholder="Unlimited" min="1" className="w-full bg-muted border border-gray-200 dark:border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder-gray-400 focus:outline-none focus:border-[#f97316]" />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Expiry Date</label>
              <input type="date" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} min={new Date().toISOString().slice(0, 10)} className="w-full bg-muted border border-gray-200 dark:border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-[#f97316]" />
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={handleCreate} disabled={saving} className="flex-1 bg-[#f97316] hover:opacity-90 text-white text-sm font-semibold py-2.5 rounded-lg transition-opacity disabled:opacity-50">
              {saving ? 'Creating…' : 'Create Code'}
            </button>
            <button onClick={() => setShowForm(false)} className="flex-1 bg-muted hover:bg-gray-200 text-foreground text-sm font-medium py-2.5 rounded-lg transition-colors">
              Cancel
            </button>
          </div>
        </div>
      ...
      {promoCodes.length === 0 && !showForm ? (
        ...
        <div className="space-y-1.5">
          {promoCodes.map(p => {
            ...
            return (
              <div key={p.id} className="bg-white dark:bg-card border border-gray-200 dark:border-border rounded-lg p-2.5">
