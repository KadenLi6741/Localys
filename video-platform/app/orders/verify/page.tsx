    ...
    return (
      <div className="min-h-screen bg-transparent text-white flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center">
          ...
          <Link href="/login" className="bg-[#f97316] hover:bg-[#ea6a0c] text-white font-semibold px-6 py-3 rounded-lg">
            Sign In
          </Link>
        </div>
      </div>
      ...
      <div className="w-full px-4 lg:px-12 py-8">
        {success ? (
          <div className="text-center">
            <svg className="w-20 h-20 mx-auto text-[#f97316] mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <h1 className="text-3xl font-bold text-[#f97316] mb-2">Order Completed!</h1>
            <p className="text-white/60 mb-6">The customer has been notified.</p>
            {order && (
              <div className="bg-white/5 border border-[#f97316]/30 rounded-lg p-4 mb-6">
                <p className="text-white font-medium">{order.item_name}</p>
                <p className="text-[#f97316] font-bold">${order.price.toFixed(2)}</p>
                <p className="text-white/40 text-xs mt-1">Order #{order.id.substring(0, 8)}</p>
              </div>
            )}
            <div className="space-y-3">
              <Link href="/dashboard" className="block w-full bg-[#f97316] hover:bg-[#ea6a0c] text-white font-semibold py-3 rounded-lg transition-colors">
                Back to Dashboard
              </Link>
          ...
          <div className="text-center">
            ...
            <button
              ...
              className="w-full bg-[#f97316] hover:bg-[#ea6a0c] disabled:bg-gray-600 text-white font-semibold py-4 rounded-lg transition-colors disabled:cursor-not-allowed flex items-center justify-center gap-2 text-lg"
            >
              {completing ? (
                <>
