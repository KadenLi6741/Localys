  ...
  return (
    <div className="min-h-screen bg-white text-[#1A1A1A] pb-24 lg:pb-8">
      <div className="w-full px-4 lg:px-12 py-4">
        {/* Header */}
        <div className="mb-6">
          <Link href="/" className="text-[var(--color-body-text)] hover:text-[var(--color-cream)] mb-4 inline-flex items-center gap-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] rounded-lg">
            ← Back
          </Link>
        ...
        {items.length === 0 ? (
          <div className="text-center py-16">
            ...
            <Link href="/" className="inline-block bg-[var(--primary)] hover:bg-[var(--primary)]/90 text-black font-semibold rounded-xl px-6 py-3 transition-colors active:scale-95">
              Browse Services
            </Link>
          </div>
        ) : (
          <>
            ...
            <div className="space-y-3 mb-6">
              {items.map((item) => (
                <div
                  key={item.itemId}
                  className="list-item-stagger bg-[var(--color-charcoal-light)] border border-[var(--color-charcoal-lighter-plus)] rounded-2xl p-4 hover:border-[var(--primary)]/30 transition-all duration-200"
                >
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[var(--primary)] font-bold">${(item.itemPrice * item.quantity).toFixed(2)}</p>
                    </div>
                    <button
                      onClick={() => removeFromCart(item.itemId)}
                      className="text-[#E05C3A] hover:text-[#E05C3A]/80 p-2 self-start rounded-lg hover:bg-[#E05C3A]/10 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]"
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    ...
                    <div className="flex items-center gap-0 bg-[var(--color-charcoal)] border border-[var(--color-charcoal-lighter-plus)] rounded-xl overflow-hidden">
                      <button
                        ...
                        className="px-3 py-1.5 text-[var(--color-cream)] hover:bg-[var(--color-charcoal-lighter)] disabled:text-[var(--color-body-text)]/40 disabled:hover:bg-transparent transition-colors min-w-[44px] min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]"
                      ...
                      <button
                        onClick={() => updateQuantity(item.itemId, item.quantity + 1)}
                        className="px-3 py-1.5 text-[var(--color-cream)] hover:bg-[var(--color-charcoal-lighter)] transition-colors min-w-[44px] min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]"
                  ...
                  <div className="mt-3">
                    <input
                      ...
                      className="w-full bg-[var(--color-charcoal)] border border-[var(--color-charcoal-lighter-plus)] rounded-xl px-3 py-2 text-sm text-[var(--color-cream)] placeholder-[#9E9A90]/50 focus:outline-none focus:border-[var(--primary)] focus:ring-1 focus:ring-[var(--primary)]/30 transition-colors"
            ...
            {loadingCoupons && (
              <div className="bg-[var(--color-charcoal-light)] border border-[var(--color-charcoal-lighter-plus)] rounded-2xl p-4 mb-6 text-center">
                <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[var(--primary)] mx-auto"></div>
              </div>
            )}
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full bg-[var(--primary)] hover:bg-[var(--primary)]/90 text-black font-semibold py-3 rounded-xl transition-all duration-200 active:scale-[0.98] shadow-lg shadow-[var(--primary)]/20 min-h-[48px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-charcoal)]"
              >
                Proceed to Checkout
              </button>
              <button
                onClick={clearCart}
                className="w-full bg-[var(--color-charcoal-light)] hover:bg-[var(--color-charcoal-lighter)] text-[var(--color-body-text)] hover:text-[var(--color-cream)] font-semibold py-3 rounded-xl transition-all duration-200 border border-[var(--color-charcoal-lighter-plus)] min-h-[48px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]"
              >
                Clear Cart
              </button>
            </div>
