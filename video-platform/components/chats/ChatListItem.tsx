...
export function ChatListItem({ chat, onClick }: ChatListItemProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full text-left flex items-center gap-4 px-4 py-5 border-b border-border transition-colors duration-150 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
        unreadCount > 0
    ...
    >
      ...
      <div className={`relative w-14 h-14 rounded-full flex-shrink-0 overflow-hidden ring-2 transition-all duration-200 ${
        ...
        {unreadCount > 0 && (
          <div className="absolute top-0 right-0 w-3 h-3 bg-primary rounded-full border border-background animate-pulse"></div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {isVerifiedSeller && (
            <span className="flex items-center gap-1 bg-success/20 border border-success/40 text-success text-xs rounded-[4px] px-2 py-0.5 whitespace-nowrap font-semibold flex-shrink-0">
      ...
      <div className="text-right flex-shrink-0">
        ...
        {unreadCount > 0 && (
          <span className="bg-primary text-primary-foreground text-xs rounded-[4px] px-2.5 py-1 inline-block font-bold animate-pulse">
