...
export function PromotionModal({ isOpen, onClose, videoId, userCoins, onSuccess, onConfirmUpload }: PromotionModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-[var(--color-charcoal)]/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-transparent border border-[var(--glass-border)] rounded-lg p-6 max-w-md w-full">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-[var(--text-primary)]">Promote Video</h2>
          <button
            onClick={onClose}
        ...
        {/* Coin Balance */}
        <div className="mb-6 p-3 bg-[var(--glass-bg-subtle)] border border-[var(--glass-border)] rounded-lg">
          <div className="flex justify-between items-center">
            <span className="text-[var(--text-secondary)]">Your Coins</span>
        ...
        <div className="mb-6">
          <div className="flex justify-between mb-2">
            <label className="text-[var(--text-primary)] text-sm font-semibold">Coins to Spend</label>
          ...
          <input
            ...
            disabled={loading}
            className="w-full h-2 bg-[var(--glass-bg-strong)] rounded-lg appearance-none cursor-pointer accent-yellow-400"
          />
            ...
            <button
              ...
              className={`py-2 rounded-lg font-semibold transition-all ${
                coinsToSpend === amount
                  ...
                  : amount > userCoins
                  ? 'bg-[var(--glass-bg-subtle)] text-[var(--text-tertiary)] cursor-not-allowed'
        ...
        <div className="mb-6 space-y-2 text-xs">
          <div className="flex items-center gap-2 text-[var(--text-secondary)]">
            ...
            <div className="flex-1 h-2 bg-[var(--glass-bg)] rounded" style={{ width: '20%' }} />
            <span>Small boost</span>
          </div>
          <div className="flex items-center gap-2 text-[var(--text-secondary)]">
            <span className="w-16">50-250 coins</span>
            <div className="flex-1 h-2 bg-[var(--glass-bg)] rounded" style={{ width: '60%' }} />
            <span>Big boost</span>
          </div>
          <div className="flex items-center gap-2 text-[var(--text-secondary)]">
            <span className="w-16">250-500 coins</span>
            <div className="flex-1 h-2 bg-[var(--glass-bg)] rounded" style={{ width: '100%' }} />
            <span>Max boost</span>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 py-3 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-strong)] text-[var(--text-primary)] rounded-lg font-semibold transition-all disabled:opacity-50"
          >
            Close
          </button>
          {isPreviewMode ? (
        ...
        <div className="mt-4 text-xs text-[var(--text-tertiary)] text-center">
          Remaining after: <span className="text-[var(--text-secondary)]">{Math.max(0, userCoins - coinsToSpend)} coins</span>
        </div>
      </div>
    </div>
  );
