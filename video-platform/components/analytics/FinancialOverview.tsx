...
const RevenueChart = dynamic(
  ...
  { ssr: false, loading: () => <div className="h-[250px] bg-surface rounded-lg animate-pulse" /> }
);

const OrdersBreakdownChart = dynamic(
  () => import('./OrdersBreakdownChart').then(m => ({ default: m.OrdersBreakdownChart })),
  { ssr: false, loading: () => <div className="h-[260px] bg-surface rounded-lg animate-pulse" /> }
);
...
export function FinancialOverview({ userId }: FinancialOverviewProps) {
  ...
  return (
    <div className="mb-8">
      ...
      >
        <span className="text-[18px] font-semibold text-foreground">Financial Overview</span>
        <svg
        ...
        <div ref={contentRef} className="bg-card border border-t-0 border-border rounded-b-2xl p-6 -mt-2">
          {loading ? (
            <div className="space-y-4">
              ...
              <div className="h-[250px] bg-surface rounded-lg animate-pulse" />
              <div className="h-[260px] bg-surface rounded-lg animate-pulse" />
            </div>
          ) : !data || data.summary.totalOrders === 0 ? (
            <div className="text-center py-12">
              <p className="text-muted-foreground text-lg mb-2">No order data yet</p>
