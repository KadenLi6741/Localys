...
const RevenueChart = dynamic(
  ...
  { ssr: false, loading: () => <div className="h-[250px] bg-[#F8F8F6] rounded-lg animate-pulse" /> }
);

const OrdersBreakdownChart = dynamic(
  () => import('./OrdersBreakdownChart').then(m => ({ default: m.OrdersBreakdownChart })),
  { ssr: false, loading: () => <div className="h-[260px] bg-[#F8F8F6] rounded-lg animate-pulse" /> }
);
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-20 bg-[#F8F8F6] rounded-xl animate-pulse" />
                ))}
              </div>
              <div className="h-[250px] bg-[#F8F8F6] rounded-lg animate-pulse" />
              <div className="h-[260px] bg-[#F8F8F6] rounded-lg animate-pulse" />
            </div>
          ) : !data || data.summary.totalOrders === 0 ? (
            <div className="text-center py-12">
              <p className="text-[#6B6B65] text-lg mb-2">No order data yet</p>
              <p className="text-[#6B6B65]/60 text-sm">
